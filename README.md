...
All you need to do is relocate the files into the appropriate folders. If you've already installed midiEdited.lua then you can skip that step. 
...
Connect the output of the node that makes up your animation or video into the midi_interpolator node, once you have done that, you will now need to specify some values. In the 'loop points' field you will need to specify the beginning and end of every loop in the input video. By default it will play one loop after another, but by checking 'use loop slider' you can then specify what loop you're looping through with the loop slider. 
...
Disable Video Rounding <br />
The math that happens when you slow down/speed up an animation may ask for a frame content that exists between 2 others. Davinci has algorithems for calculating this with video inputs (Nearest, blend, flow) which will use your project's default settings to fill in. To use a non-default mode put a 'TimeSpeed' node between this one and your video, and set your desired mode in the node's setting. Use this setting the same way you would use 'Disable Midi Rounding'.  
...
Use Loops (NOT IMPLEMENTED YET) <br />
